...
function checkOffsets() {
  ...
  const cell = gridCellDimensions();
  const elements =
      document.querySelectorAll('body :not(.debug-grid, .debug-toggle)');
  for (const element of elements) {
    ...
    if (offset > 0) {
      ...
      console.error(
          'Incorrect vertical offset for', element, 'with remainder',
          top % cell.height, 'when expecting divisible by', cell.height / 2);
    } else {
